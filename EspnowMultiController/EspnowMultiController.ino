// first 30 byte Sending Data ESPNOW CONTROLLER
// second 30 byte Receiving Data 
...
byte firstData[30] = {123,124}; //Sending 123,124 is Examples data 
byte secondData[30];  //Receiving
...
void sendData() {
  ...
  for (int i = 0; i < SlaveCnt; i++) {
    ...
    Serial.println(sendingData.text);
...
void receiveCallBackFunction(uint8_t *senderMac, uint8_t *incomingData, uint8_t len) {
    ...
    for (int i=0; i<90; i=i+3) {
      ...
      secondData[i/3] = S.toInt();
    }

    Serial.println("  Example Data");
    Serial.println(secondData[0]);
...
void loop() {
  ...
  if (SlaveCnt > 0) { // check if slave channel is defined
    ...
    // bring Data Array to sendingData.text
    String DataText = "";
    for (int i=0; i<30; i++) {
      if (firstData[i]<30) {
        DataText=DataText + " ";       
      ...
      DataText=DataText + String(firstData[i]);
    }
    DataText.toCharArray(sendingData.text,300);
  ...
  // wait for 15seconds to run the logic again
  delay(15000);
  Serial.println("Delay 15 seconds to avoid reset at Slave...");

  // Code from BasicHttpClient
  USE_SERIAL.print("WiFi change mode\n");
  esp_now_deinit();
  WiFi.disconnect();
  WiFi.mode(WIFI_OFF);
  delay(1000);
  WiFi.mode(WIFI_STA);
  delay(1000);
  WiFi.begin(ssid, password); // Your SSID , Your Password
  ...
  if ((WiFi.status() == WL_CONNECTED)) {
    ...
    // EspnowMultiServer connecting and Your Internet Connect code
    // EspnowMultiServer connecting ....

    Serial.println("nothing to do now, waiting code from user");
    // end of EspnowMultiServer connecting
    // Your Internet Connect code here....


    // Your Internet Connect code end
  }
